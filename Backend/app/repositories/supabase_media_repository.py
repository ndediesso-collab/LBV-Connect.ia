...
class SupabaseMediaRepository:
    ...
    def _attach_public_url(
        ...
        """Reconstruit l'URL publique canonique à partir de storage_path."""
        item = dict(row)
        storage_path = item.get("storage_path")

        if storage_path:
            try:
                public_url = self.get_public_url(str(storage_path))
                item["url"] = public_url
                item["media_url"] = public_url
                item["public_url"] = public_url
            except Exception:
                # Ne casse pas tout l'appelant si une URL ne peut
                # pas être reconstruite pour cette ligne.
                pass
